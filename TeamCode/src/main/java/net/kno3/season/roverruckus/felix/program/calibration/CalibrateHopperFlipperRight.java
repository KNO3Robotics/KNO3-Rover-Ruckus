...
@TeleOp(group = "calibration", name = "Calibrate Hopper Flipper Right")
public class CalibrateHopperFlipperRight extends LinearOpMode{
    @Override
    public void runOpMode() throws InterruptedException {
        ...
        while(opModeIsActive()) {
            Servo hopperFlipper = hardwareMap.servo.get(Felix.HOPPER_FLIPPER_RIGHT_KEY);
            double idleSetpoint = settings.getDouble("hopper_flipper_right_idle");
            ...
            double upSetpoint = settings.getDouble("hopper_flipper_right_up");
            while(opModeIsActive() && !gamepad1.a) {
                if(Math.abs(gamepad1.left_stick_x) > 0.05) {
            ...
            double downSetpoint = settings.getDouble("hopper_flipper_right_down");
            while(opModeIsActive() && !gamepad1.a) {
                if(Math.abs(gamepad1.left_stick_x) > 0.05) {
            ...
            double middleSetpoint = settings.getDouble("hopper_flipper_right_middle");
            while(opModeIsActive() && !gamepad1.a) {
                if(Math.abs(gamepad1.left_stick_x) > 0.05) {
            ...
            settings.setDouble("hopper_flipper_right_idle", idleSetpoint);
            settings.setDouble("hopper_flipper_right_up", upSetpoint);
            settings.setDouble("hopper_flipper_right_down", downSetpoint);
            settings.setDouble("hopper_flipper_right_middle", middleSetpoint);
            settings.save();
