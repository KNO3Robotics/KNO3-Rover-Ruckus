...
@TeleOp(group = "calibration", name = "Calibrate Hopper Bar")
public class CalibrateHopperBar extends LinearOpMode{
    @Override
    public void runOpMode() throws InterruptedException {
        ...
        while(opModeIsActive()) {
            Servo hopperFlipper = hardwareMap.servo.get(Felix.HOPPER_BAR_KEY);
            double idleSetpoint = settings.getDouble("hopper_bar_idle");
            ...
            double upSetpoint = settings.getDouble("hopper_bar_up");
            while(opModeIsActive() && !gamepad1.a) {
                if(Math.abs(gamepad1.left_stick_x) > 0.05) {
            ...
            double downSetpoint = settings.getDouble("hopper_bar_down");
            while(opModeIsActive() && !gamepad1.a) {
                if(Math.abs(gamepad1.left_stick_x) > 0.05) {
            ...
            settings.setDouble("hopper_bar_idle", idleSetpoint);
            settings.setDouble("hopper_bar_up", upSetpoint);
            settings.setDouble("hopper_bar_down", downSetpoint);
            settings.save();
